...
<div class="col-lg-12 col-md-12 col-sm-12">
			...
			<div class="col-lg-12 col-md-12 col-sm-12" style="vertical-align:bottom;">
				<div class="form-group col-lg-12 col-md-12 col-sm-12 col-lg-offset-5">
					<button type="submit" class="btn" id="submitCreate" onclick="modifyProgram()">Modify</button>
				</div>
			</div>
		</div>
</body>
<script type="text/javascript">
   ...
   $(document).ready(function() {
	   ...
	   progId='<c:out value="${parameters.progId}"></c:out>';
	   loadModifyData(progId);
    });
   ...
   function createProgram(){
	   ...
	   dataArray["output1"]=$('#output1').val();
		...
		$.ajax({
			...
			success : function(data) {
				 var paremeters = {};
				loadNewPage('/adminPages/adminMainPage.jsp',paremeters,'content')
			},
			error : function(e) {
				console.log("ERROR: ", e);
			},
   ...
   function  loadModifyData(progId){
	   var dataArray = {}
		dataArray["progId"]=progId;
		$.ajax({
			type : "POST",
			contentType : "application/json",
			url : "loadPagramDetailsForModify",
			data : JSON.stringify(dataArray),
			timeout : 100000,
			success : function(data) {
				if(data.status==0){
				   $('#typeCreate').val(data.type);
				   $('#progName').val(data.progName);
				   $('#complexityCreate').val(data.complexity);
				   $('#prog1').val(data.prog1);
